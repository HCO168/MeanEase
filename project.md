...
正式词库使用独立的 `morphology`、`morphology_source`、`morphology_license` 字段承载面向学习者的“构词与记忆拆解”。Prompt v7 `v7-maximal-learning` 优先递归拆尽真实 prefix/root/base/suffix/combining form；若真实构词无法再细分但存在有助于拼写记忆的连续片段，可显式使用 `mnemonic_anchor` 与 `spelling_fragment`，二者必须声明不代表真实现代构词义。旧 `etymology*` 字段已经从运行结构中移除，不再作为兼容字段读取。正式词库版本升级为 `20000-multisource-cefr-maximal-learning-2026-09-v5`；静态客户端用词库版本触发 IndexedDB 词典资料刷新，但只覆盖词典字段并保留逐词调度状态。云存档只保存个人进度、不含词典正文，恢复继续兼容旧 v3、v4 与当前 v5 的 `source_library_version`。2026-09-13 的正式 Luna Medium Batch 共覆盖 20,000 词；首轮 validator v3 自动安全区写入 8,778 条，后续人工复核 spelling mismatch、错误拼写说明、硬 issue、root/base warning 与生产性词缀 warning 后覆盖增至 9,743 条。2026-09-14 按最大化学习拆解规则人工将 `interrogation` 展开为 `inter- + rog + -ate + -ion`，并将 `window` 记录为 `wind`（记忆锚点）+ `ow`（拼写片段），当前正式 `morphology*` 共覆盖 9,744 词（48.72%）。未通过或仍不确定的候选继续保持空白。原始 Batch 数据与首轮落库备份在根仓库 `discard/20260913-meanease-morphology-production/`；v7 修改前 9,743 条正式 CSV 另备份在 `discard/20260914-meanease-maximal-learning-v7/`。

### 批量构词草稿

当前生成规则为 Prompt v7 `v7-maximal-learning`：目标从“保守只收严格现代词素”调整为“在不冒充词素的前提下最大化学习拆解”。例如 `interrogation` 应优先展开为 `inter- + rog + -ate + -ion`；`window` 可表示为 `wind`（记忆锚点）+ `ow`（拼写片段）。纯记忆片段必须在类型与中文解释中明确标记，不能伪装成 root/base/suffix。

构词批处理使用 OpenAI Batch API 的 `/v1/responses`，默认模型为 `gpt-5.6-luna`，也可通过 `--model` 指定其他兼容模型。脚本会按模型选择默认 reasoning：Luna 使用 `none`，`gpt-5-nano` 使用 `minimal`；也可用 `--reasoning-effort` 显式覆盖。默认每个请求 25 个词，`max_output_tokens=20000`（该预算同时覆盖 reasoning tokens 与最终 JSON 输出），并通过 Structured Outputs 返回 `status / parts / spelling_note / confidence`。提示词使用少量代表性 few-shot 示例说明可靠前缀/词根/后缀、拼写变化与不可拆情形，同时保留可泛化判定原则；benchmark 使用与这些示例完全不重合的 holdout 词，避免训练示例直接出现在测试集中。模型可识别现代基础词、bound root、前缀、后缀和 combining form，不限制成分数量或组合方式；选择标准是构词成分真实、可复用、当前语义成立且对学习有价值。输入只包含静态词典中的单词、基础词、词性和中文释义，不包含个人学习数据。

```bash
python3 morphology_luna_batch.py prepare
# Nano 试跑示例：python3 morphology_luna_batch.py prepare --model gpt-5-nano
...
- `morphology*` 已完成结构切换；首轮自动安全候选 8,778 条加上 `spelling_mismatch_without_note` 人工复核新增 735 条，当前共 9,743 个词有学习型构词拆解，覆盖 48.72%；其余词保持空白等待后续复核或明确不可拆；
