...
2. 不预设拆解深度。你可以保留现代英语基础词整体，也可以继续完整拆到稳定的 bound root、前缀、后缀或连续派生成分；根据哪个粒度最能准确、清楚地展示真实构词关系来决定。
3. 每个被拆出的成分都必须是稳定、可复用、具有明确构词身份的学习型成分。不得为了让字母恰好拼接而临时创造只适用于当前单词的“词根”“基础词”或任意字母片段。
4. 不要因为存在一个可识别的现代基础词就禁止进一步拆解，也不要因为理论上还能继续切分就强制完全拆解。允许模型在多个可靠层级之间自由选择最有学习价值的分析。
...
B. 枚举合理的拆解层级，包括基础词层级和更完整的词根/词缀层级，不人为偏好更浅或更深。
C. 对每个候选成分逐一验证：它是否是真实稳定的构词成分、类型是否合理、在当前单词中的语义是否成立、整体是否能解释目标词。
D. 在多个都可靠的分析之间，自由选择最有助于学习者理解构词关系的一种；完整拆解是允许的。
