...
默认词库基于 [ECDICT](https://github.com/skywind3000/ECDICT) 构建。难度先采用 [Oxford 3000/5000](https://www.oxfordlearnersdictionaries.com/us/wordlists/oxford3000-5000)，再用 [CEFR-J 1.5 和 Octanove C1/C2 1.0](https://github.com/openlanguageprofiles/olp-en-cefrj) 补足；只有被 Octanove 明确标为 C2 的词才记录为 Beyond C1，未匹配词保持未定级。C2 定级阶段与定级后的新词均采用约 75% 明确 C2 词和 25% C1 校准/巩固词，未定级词不参与定级测试。正式词库使用独立 `morphology`、`morphology_source`、`morphology_license` 字段承载构词资料；旧 v3/v4 个人进度云存档继续兼容，当前产品词库版本与发布状态以 HCONet 的 `doc/frontend/meanease pages.md` 为准。双语例句来自 [Tatoeba](https://tatoeba.org/) 的校对语料。定级结果只用于安排学习范围，不是正式 CEFR 认证。

2026-09-11 的 83 批人工构词审校响应保留在 `morphology_reviews/2026-09-11/`。它们采用旧 `etymology*` 字段，仅供审计原始证据和旧快照；不能直接覆盖当前 `morphology*` 正式词库。
